...
func deflect_from(point: Vector2, speed_mult: float = 1.0, spread_deg: float = 0.0) -> void:
	...
	if _velocity.length() < 1.0:
		_velocity = n * maxf(speed, 60.0)

	_velocity *= maxf(speed_mult, 1.0)

	# פיזור קטן (כדי שזה ירגיש “ריקושט” ולא רק היפוך מושלם)
...
func _on_area_entered(area: Area2D) -> void:
	...
	# אם זה כדור מוחזר - הוא אמור לפגוע באויבים
	if _deflected:
	...
	# אם זה כדור אויב שפגע בשחקן - נעלם (השחקן מוריד חיים בצד שלו)
